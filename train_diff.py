...
class myTrain(LightningModule):
    def __init__(self, cfg, log_dir = None):
        ...
        self.test_max_f1 = [0 for _ in range(20)]
        self.best_f1_diff = float('inf')
        self.best_f1_diff_epoch = -1
        self.best_f1_diff_log = None
        self.test_loaders = [
            build_dataloader(cfg.dataset_config1, mode='test'),
            build_dataloader(cfg.dataset_config2, mode='test')
        ]
    ...
    def train_dataloader(self):
        loader = build_dataloader(self.cfg.dataset_config2, mode='train')
        return loader

    def val_dataloader(self):
        loader = build_dataloader(self.cfg.dataset_config2, mode='val')
        return loader
...
class myTrain(LightningModule):
    ...
    def on_validation_epoch_end(self):
        ...
        for idx in range(0, len(self.cfg.monitor_test), 1):
            ...
            if self.cfg.argmax:
                f1_1 = self.test(idx, self.test_loaders[0])
                f1_2 = self.test(idx + 10, self.test_loaders[1])
                self.log(f'{self.cfg.monitor_test[idx]}_set1', f1_1, on_step=False, on_epoch=True, prog_bar=True)
                self.log(f'{self.cfg.monitor_test[idx]}_set2', f1_2, on_step=False, on_epoch=True, prog_bar=True)

            else:
                t = 0.2 + 0.1 * idx
                f1_1 = self.test(idx, self.test_loaders[0], t)
                f1_2 = self.test(idx + 10, self.test_loaders[1], t)
                self.log(f'{self.cfg.monitor_test[idx]}_set1', f1_1, on_step=False, on_epoch=True, prog_bar=True)
                self.log(f'{self.cfg.monitor_test[idx]}_set2', f1_2, on_step=False, on_epoch=True, prog_bar=True)
            
            f1_diff = abs(f1_2 - f1_1)
            self.log("f1_diff", f1_diff, on_step=False, prog_bar=True, on_epoch=True)


    def test(self, idx, test_loader, value = None):
...
if __name__ == "__main__":
    ...
    trainer.fit(model, ckpt_path=cfg.resume_ckpt_path)shape[2:]).to('cuda')).long()
            param = 1  # This parameter is balance precision and recall to get higher F1-score
            preds[:,1,:,:] = preds[:,1,:,:] + param 
